...
void GraphWindow::drawCurve(QPainter &painter, const QRect &plotArea) {
  ...
  double xStep = (m_maxIteration > 0)
                     ? plotArea.width() / static_cast<double>(m_maxIteration)
                     : 0;
  double yScale = (m_maxValue - m_minValue > 1e-10)
                      ? plotArea.height() / (m_maxValue - m_minValue)
                      : 1.0;
